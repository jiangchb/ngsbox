...
while( my $line = <VCF> ) {
	...
	if($line =~ /^#/) {
		...
		### Default: $snp_support >= 3 && $anno{DP} >=5 && $anno{DP} <= 250 && $anno{AF} >= 0.001 && $anno{RE} <= 1.3 && $a[5] >= 20
		if( $snp_support >= 3 && $anno{DP} >=5 && $anno{DP} <= $max_depth && $anno{AF} >= 0.001 && $anno{RE} <= 1.3 && $a[5] >= 20 ) { 
			print "$line\n";
		}
	}
}
