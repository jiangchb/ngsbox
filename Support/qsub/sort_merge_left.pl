...
#  -------------------------------------------------------------------------
#
#  Module: Support::qsub::sort_merge_left.pl
#  Purpose:
#  In:
#  Out:
#
...
foreach my $infolder (@infolders) {
	...
	foreach my $typefolder (@typefolders) {
		...
		foreach my $lengthfolder (@lengthfolders) {
			...
			if($typeleaf eq "1") {
				if(-e "$lengthfolder/left_over.fl") {
					$files_r1 .= "$lengthfolder/left_over.fl,"
				}
				elsif(-e "$lengthfolder/left_over.fl.gz") {
					$files_r1 .= "$lengthfolder/left_over.fl.gz,"
				}
			}
			elsif($typeleaf eq "2") {
				if(-e "$lengthfolder/left_over.fl") {
					$files_r2 .= "$lengthfolder/left_over.fl,"
				}
				elsif(-e "$lengthfolder/left_over.fl.gz") {
					$files_r2 .= "$lengthfolder/left_over.fl.gz,"
				}
			}
		}	
	}
