...
while (my $ref = $sth->fetchrow_hashref()) {
	...
	my $enst_fwd = "NA";
	my $ann_UCSC_known_fwd = $dbh->selectall_hashref("SELECT * FROM ann_UCSC_Ensembl WHERE strand = '+' AND chrom = 'chr$chr' AND txStart >= $peak_middle ORDER by txStart ASC limit 1", "name");
	foreach my $id (keys %$ann_UCSC_known_fwd) {
		$enst_fwd = $ann_UCSC_known_fwd->{$id}->{name};
	}

	### Get Ensembl rev downstream gene
	my $enst_rev = "NA";
	my $ann_UCSC_known_rev = $dbh->selectall_hashref("SELECT * FROM ann_UCSC_Ensembl WHERE strand = '-' AND chrom = 'chr$chr' AND txStart <= $peak_middle ORDER by txStart DESC limit 1", "name");
	foreach my $id (keys %$ann_UCSC_known_rev) {
		$enst_rev = $ann_UCSC_known_rev->{$id}->{name};
	}
	...
	print NDG $enst_fwd ."\t". $ann_UCSC_known_fwd->{$enst_fwd}->{name2} ."\t";
	...
	print NDG $enst_rev ."\t". $ann_UCSC_known_rev->{$enst_rev}->{name2} ."\t";
