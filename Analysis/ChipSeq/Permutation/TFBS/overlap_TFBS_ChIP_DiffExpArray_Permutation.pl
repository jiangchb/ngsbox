...
#  -------------------------------------------------------------------------
#
#  Module: Analysis::ChipSeq::Permutation::TFBS::overlap_TFBS_ChIP_DiffExpArray_Permutation.pl
#  Purpose:
#  In:
#  Out:
#
...
while( my $ref0 = $sth0->fetchrow_hashref() ) {
	...
	$sth->finish();
        ...
        ### Load ChIPseq peaks
        $q = "	SELECT distinct begin, end, name
                FROM    $chip_table
                WHERE   chr = $chr &&
                        max_cov >= $minpeak &&
	...
	while(my $ref = $sth->fetchrow_hashref()) {
		...
		if($hit == 1) { 
			$real_result{good}++; 
			print GENE "$chr_alias:$beg..$end\t" . $ref->{name} . "\n";
		}
		else { 
			$real_result{bad}++;
		}
...
\n");
        ...
        GetOptions(\%CMD, "db=s", "chip=s", "diffexp=s", "tfbs=s", "permnum=s", "pref=s", 
			"minpeak=s", "minlen=s", "cons=s", "pats=s", "rep=s", "range=s",
