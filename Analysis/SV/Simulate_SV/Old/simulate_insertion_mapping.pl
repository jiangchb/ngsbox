...
#  -------------------------------------------------------------------------
#
#  Module: Analysis::SV::Simulate_SV::Old::simulate_insertion_mapping.pl
#  Purpose:
#  In:
#  Out:
#
...
my $usage = "simulate_insertion_mapping.pl num";
...
# Create deletions
open STAT, "> insertions.statistics.".$min."-".$max.".txt";
open LOG, "> insertions.".$min."-".$max.".txt";
open INSERTSIZES, "> insertions.insertsizes.".$min."-".$max.".txt";
open DISCORDANT_INSERTSIZES, "> insertions.insertsizes_discordant.".$min."-".$max.".txt";
...
for (my $i = 1; $i <= $num; $i++) {
	...
	my $start = int(rand(3000)) + $offset; # start is between 2000 and 5000
	my $length = int(rand($max-$min))+$min;  
	my $rand_seq = "";
	my @nucl_arr=("A", "C", "T", "G");
	for (my $nucl = 0; $nucl < $length; $nucl++) {
		$rand_seq .= $nucl_arr[int(rand(4))];
	}
	my $sample_seq = substr($reference, 0, $start).$rand_seq.substr($reference, $start+1, length($reference)-($start-1));
	...
	######################################################################################################################################################
	# Parse output
	open OUTPUT, "sv_pred/sv.$i.out/SHORE_SV_insertion.pe.txt";
	...
	print LOG "\t", $pred_start, "\t", $pred_end, "\t", $pred_length, "\t", $pred_gap, "\n";
	close OUTPUT;
