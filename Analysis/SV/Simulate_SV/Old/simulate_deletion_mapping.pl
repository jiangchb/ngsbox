...
#  -------------------------------------------------------------------------
#
#  Module: Analysis::SV::Simulate_SV::Old::simulate_deletion_mapping.pl
#  Purpose:
#  In:
#  Out:
#
...
my $usage = "simulate_deletion_mapping.pl num";
...
# Create deletions
open STAT, "> deletions.statistics.".$min."-".$max.".txt";
open LOG, "> deletions.".$min."-".$max.".txt";
open INSERTSIZES, "> deletions.insertsizes.".$min."-".$max.".txt";
open DISCORDANT_INSERTSIZES, "> deletions.insertsizes_discordant.".$min."-".$max.".txt";
...
for (my $i = 1; $i <= $num; $i++) {
	...
	my $sample_seq = substr($reference, 0, $start).substr($reference, $start+$length, length($reference)-($start+$length+1));
	...
	######################################################################################################################################################
	# Parse output
	open OUTPUT, "sv_pred/sv.$i.out/SHORE_SV_deletion.pe.txt";
