...
#  -------------------------------------------------------------------------
#
#  Module: Analysis::Validation::PCR::ma_validation.pl
#  Purpose:
#  In:
#  Out:
#
...
### Get SNPs
my $q ="SELECT chromosome, position, ma, ref_base, kind 
	FROM putants_v3_norman
...
while( my $ref = $sth->fetchrow_hashref() ) {
	...
	my $putant_line = $ref->{ma};
	my $reference   = $ref->{ref_base};
	my $putation    = $ref->{kind};
	...
	print ">$chr-$start-$end(Putant=MA_$putant_line-$pos-$reference-$putation-pvalue=nan):TARGET=450,101\n";
